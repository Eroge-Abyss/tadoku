...
export function usePlaytimeStats(getNovel: () => Novel) {
  ...
  const lastPlayedDate = $derived(
    novel.last_played ? new SvelteDate(novel.last_played * 1000) : null,
  );
  const firstPlayedDate = $derived(
    novel.first_played ? new SvelteDate(novel.first_played * 1000) : null,
  );
  ...
  return {
    ...
    get todayHoursPlayed() {
      return todayHoursPlayed;
    },
    get todayMinutesPlayed() {
      return todayMinutesPlayed;
    },
    get lastPlayedDate() {
